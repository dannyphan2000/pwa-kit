...
describe('parseJSDoc', () => {
  ...
  it('handles missing pieces', () => {
    expect(parseJSDoc('* Only desc'))
      .toMatchObject({description: 'Only desc', params: [], returns: '', example: ''})
  })
})

describe('parseTypeScriptParameters & parseOptionsParameter', () => {
  const {parseTypeScriptParameters, parseOptionsParameter} = jest.requireActual('./explore-doc-tool.js')
  ...
  it('parses options with parameters and headers', () => {
    const str = 'options: { parameters?: { foo: string, bar?: number }, headers?: { Auth: string } }'
  ...
  it('fallbacks for unusual params', () => {
    expect(parseTypeScriptParameters('id: string, opts?: boolean')).toMatchObject({raw: 'id: string, opts?: boolean'})
  })
...
describe('parseReturnType & extractReturnTypeStructure', () => {
  const {parseReturnType, extractReturnTypeStructure} = jest.requireActual('./explore-doc-tool.js')
  ...
  it('attempts to extract for interfaces/types in file', () => {
    ...
    expect(resultI.properties && resultI.properties.find((p) => p.name === 'email')).toBeUndefined()
